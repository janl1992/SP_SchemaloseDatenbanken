...
Für den Import vom Client wird das psql-Statement \textbackslash copy verwendet (siehe SQL Script \ref{2.copy.listing}).
...
profiles Tabelle. ID hingegen ist in der profiles Tabelle ein Primärschlüssel (siehe \ref{2.facebookProfiles.listing}).
...
der temporären Tabelle in die endgültige profile Tabelle schreibt (siehe hierzu auch beispielhaft das Script für die Tabelle facebook-profiles (siehe \ref{2.foreignKey.listing}).
...
Script, was die Tabelle relation$\_$facebook mit Hilfe des WITH RECURSIVE Befehl bis zur Rekursionsstufe 5 traversiert (siehe \ref{2.StandardSQL.listing}),
sowie ein Script, welches den Standard SQL Source Code dynamisch generiert (siehe \ref{2.StandardSQLGenerisch.listing}).
...
befindet sich im Anhang (siehe \ref{2.SELECT.listing}). Auf der obersten Rekursionsstufe wird der Startknoten des Graphen mitgegeben (in diesem Beispiel ist der Startknoten = 1).
...
der momentanen Rekursionsstufe entfernt werden. Die Funktionsweise von DISTINCT ist in der folgenden Grafik nochmal dargestellt:
\begin{figure}[H]
...
Der Ausführungsplan für ein verschachteltes SELECT der Tiefe 5 befindet sich im Anhang (siehe \ref{2.AusführungsplanCascadeSELECT.listing}). Für die DISTINCT Überprüfung
...
\begin{lstlisting}[language=SQL,caption = IndexScanFacebookRelation,frame=single, label={2.indexScanFacebookRelation.listing} ]
    ...
    Index Cond: (src = 765)
\end{lstlisting}
Dieser wird nur auf der ersten Rekursionsstufe verwendet. Auf den tieferen Rekursionsstufen wird immer ein Sequential Scan vollzogen (vergleiche hierzu \ref{2.AusführungsplanCascadeSELECT.listing} ). Der Grund dafür ist, dass nur auf der ersten Rekursionsstufe auf
...
befinden. Ein Beispielstatement für den rekursiven INNER JOIN ist im Anhang gegeben (siehe \ref{2.JOIN.listing}). Der Ausführungsplan für die relation$\_$facebook Tabelle befindet sich
im Anhang (\ref{2.AusführungsplanINNERJOIN.listing}). Fast alle Verknüpfungen werden mit Hilfe eines Hash Join vollzogen.Bei dem Ausführungsplan fällt der Merge Join auf, der sehr viel Zeit in Ansprich nimmt:
...
zu einer mit gegebenen Rekursionstiefe wieder aufruft, traversiert (siehe \ref{2.recursiveFunction.listing}). Die \textcolor{red}{Abbruchbedingung} wird dem Stored Procedure in Form einer
